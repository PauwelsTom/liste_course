...
def all(app, get_db):
    ...
    @app.put("/ingredients/{foyer}", response_model=Schemas.Ingredients.Ingr)
    def update_ingredient_route(foyer: int, ingredient: Schemas.Ingredients.IngrCreate, db: Session = Depends(get_db)):
        return Crud.Ingredients.update_ingr(db=db, ingr=ingredient, foyer=foyer)

    @app.post("/ingredients/{foyer}", response_model=Schemas.Ingredients.Ingr)
    ...
    @app.get("/ingredients/{foyer}", response_model=list[Schemas.Ingredients.Ingr])
    def get_ingredients_route(foyer: int, db: Session = Depends(get_db)):
        return Crud.Ingredients.get_ingredients(db, foyer)

    @app.get("/ingredient/{ingr_name}", response_model=Schemas.Ingredients.Ingr)
