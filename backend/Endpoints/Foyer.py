...
def all(app, get_db):
    @app.get("/foyer/", response_model=list[Schemas.Foyer.Foyer], tags=["Foyer"])
    def get_foyers(db: Session = Depends(get_db)):
        return Crud.Foyer.get_foyers(db=db)

    @app.post("/foyer/", response_model=Schemas.Foyer.Foyer, tags=["Foyer"])
    ...
    @app.put("/foyer/", response_model=Schemas.Foyer.Foyer, tags=["Foyer"])
    ...
    @app.delete("/foyer/", response_model=bool, tags=["Foyer"])
